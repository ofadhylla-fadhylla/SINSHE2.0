...
function dateMs(value){if(!value)return null;const d=new Date(`${isoDate(value)}T12:00:00`);return Number.isNaN(d.getTime())?null:d.getTime()}
function ageDays(value){const ms=dateMs(value);if(ms===null)return null;return Math.floor((Date.now()-ms)/86400000)}
function recordDate(row,...names){return field(row,...names,'updated_at','created_at')}
function inAgeWindow(row,minAge,maxAge,...names){const age=ageDays(recordDate(row,...names));return age!==null&&age>=minAge&&age<maxAge}
function trendPct(current,previous){if(previous===0)return current>0?100:0;return Math.round(((current-previous)/previous)*100)}
export function predictionConfidence({records=0,sources=0,maxSources=5}={}){
  const sourceScore=Math.min(60,(sources/Math.max(1,maxSources))*60)
  const volumeScore=Math.min(40,Math.log10(Math.max(1,records)+1)*18)
  const score=clamp(sourceScore+volumeScore)
  return {score,label:score>=75?'High':score>=45?'Medium':'Low',tone:score>=75?'green':score>=45?'blue':'orange'}
}
...
export function assetHealth(asset){
  ...
  if(monthly&&typeof monthly==='object'){
    ...
    const bad=values.filter(v=>/rusak|bad|ng|abnormal|repair|breakdown/.test(v)).length
    if(bad>=2)score-=25
    else if(bad===1)score-=18
  }
  return clamp(score)
}

export function assetDrivers(asset){
  const out=[]
  const cert=String(field(asset,'certificate_status')||'').toLowerCase(),test=String(field(asset,'test_status')||'').toLowerCase(),d=daysTo(field(asset,'next_test_date','riksa_due','silo_due'))
  if(!field(asset,'certificate_no')&&!cert.includes('bersertifikat'))out.push('Certificate/SILO belum tersedia')
  if(cert.includes('tidak'))out.push('Status tidak bersertifikat')
  if(test.includes('expired')||(d!==null&&d<0))out.push('Riksa uji expired/overdue')
  else if(test.includes('belum'))out.push('Riksa uji belum ada')
  else if(d!==null&&d<=30)out.push(`Riksa uji jatuh tempo ${Math.max(0,d)} hari`)
  else if(d!==null&&d<=90)out.push(`Riksa uji jatuh tempo ${d} hari`)
  if(String(field(asset,'operational')).toLowerCase()==='inactive')out.push('Asset inactive')
  const monthly=field(asset,'condition_monthly')||{}
  if(monthly&&typeof monthly==='object'&&Object.values(monthly).some(v=>/rusak|bad|ng|abnormal|repair|breakdown/i.test(String(v||''))))out.push('Abnormal condition history')
  return out.length?out:['Routine condition monitoring']
}
...
export function assetPriority(asset){
  ...
  const monthly=field(asset,'condition_monthly')||{}
  if(monthly&&typeof monthly==='object'){
    const bad=Object.values(monthly).filter(v=>/rusak|bad|ng|abnormal|repair|breakdown/i.test(String(v||''))).length
    score+=Math.min(20,bad*8)
  }
  return clamp(score)
}

export function assetPriorityDetail(asset){
  const score=assetPriority(asset),health=assetHealth(asset),drivers=assetDrivers(asset)
  const usableSignals=[field(asset,'certificate_no'),field(asset,'certificate_status'),field(asset,'test_status'),field(asset,'next_test_date','riksa_due','silo_due'),field(asset,'condition_monthly')].filter(Boolean).length
  const confidence=predictionConfidence({records:usableSignals,sources:Math.min(4,usableSignals),maxSources:4})
  return {score,health,drivers,confidence,window:inspectionWindow(score)}
}
...
export function companyPrediction(code,data){
  ...
  const recentObs=obs.filter(r=>inAgeWindow(r,0,90,'observation_date'))
  const inc30=incidents.filter(r=>inAgeWindow(r,0,30,'incident_date')).length,incPrev=incidents.filter(r=>inAgeWindow(r,30,60,'incident_date')).length
  const obs30=obs.filter(r=>inAgeWindow(r,0,30,'observation_date')).length,obsPrev=obs.filter(r=>inAgeWindow(r,30,60,'observation_date')).length
  const action30=actions.filter(r=>inAgeWindow(r,0,30,'updated_at')).length,actionPrev=actions.filter(r=>inAgeWindow(r,30,60,'updated_at')).length
  const incidentTrend=trendPct(inc30,incPrev),observationTrend=trendPct(obs30,obsPrev),actionTrend=trendPct(action30,actionPrev)
  const trendPressure=Math.max(0,incidentTrend)*0.08+Math.max(0,observationTrend)*0.03+Math.max(0,actionTrend)*0.02
  const base=highInc.length*14+openInc.length*5+overdueActions.length*9+complianceRisk.length*8+assetRisk.length*1.5+recentObs.length*2
  const score=clamp(base+Math.min(18,trendPressure))
  const sourceLists=[incidents,obs,actions,compliance,assets],sources=sourceLists.filter(v=>v.length).length,records=sourceLists.reduce((s,v)=>s+v.length,0),confidence=predictionConfidence({records,sources,maxSources:5})
  const reasons=[]
  if(highInc.length)reasons.push(`${highInc.length} high/critical incident open`)
  if(overdueActions.length)reasons.push(`${overdueActions.length} corrective action overdue`)
  if(complianceRisk.length)reasons.push(`${complianceRisk.length} compliance item high risk`)
  if(assetRisk.length)reasons.push(`${assetRisk.length} asset inspection priority tinggi`)
  if(incidentTrend>0)reasons.push(`incident 30-day trend +${incidentTrend}%`)
  if(!reasons.length&&recentObs.length)reasons.push(`${recentObs.length} observation 90 hari terakhir`)
  return {code,score,label:riskLabel(score),openIncidents:openInc.length,overdueActions:overdueActions.length,complianceRisk:complianceRisk.length,assetRisk:assetRisk.length,recentObservations:recentObs.length,incidentTrend,observationTrend,actionTrend,sourceCoverage:sources,recordCount:records,confidence,reasons:reasons.slice(0,3)}
}
