const projectUrl = (process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://synmhawsidogugkverxk.supabase.co').replace(/\/$/, '')
const publishableKey = process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || 'sb_publishable_NJJ13npkMJDbEP4b5gKaaA_gWl9n-Nd'
...
export async function provisionInitialAdmin(email, password) {
  assertConfigured()
  const response = await fetch(`${projectUrl}/functions/v1/provision-initial-admin`, {
    method: 'POST',
    headers: {
      ...authHeaders(),
...
export async function storageUpload(bucket, path, file, { upsert = false } = {}) {
  assertConfigured()
  const session = await getValidSession()
  if (!session?.access_token) throw new Error('Sesi login tidak tersedia.')
  if (!file) throw new Error('File tidak tersedia.')
  const response = await fetch(`${projectUrl}/storage/v1/object/${storagePath(bucket, path)}`, {
    method: 'POST',
    headers: {
      ...authHeaders(session.access_token),
...
export async function storageSignedUrl(bucket, path, expiresIn = 3600) {
  ...
  const session = await getValidSession()
  if (!session?.access_token) throw new Error('Sesi login tidak tersedia.')
  const response = await fetch(`${projectUrl}/storage/v1/object/sign/${storagePath(bucket, path)}`, {
    method: 'POST',
    headers: {
      ...authHeaders(session.access_token),
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ expiresIn: Math.max(60, Number(expiresIn) || 3600) }),
  })
  const body = await parseResponse(response)
  const signed = body?.signedURL || body?.signedUrl || ''
  if (!signed) throw new Error('Signed URL tidak tersedia.')
  if (/^https?:\/\//i.test(signed)) return signed
  if (signed.startsWith('/storage/v1')) return `${projectUrl}${signed}`
  return `${projectUrl}/storage/v1${signed.startsWith('/') ? signed : `/${signed}`}`
}
