...
export function Progress({value, tone='green'}) {
  return <div className="progress"><span className={`p-${tone}`} style={{width: `${Math.max(0, Math.min(100, Number(value) || 0))}%`}}/></div>
}
...
export function BarList({data=[]}) {
  ...
  return <div className="bar-list">
    {rows.map(d => <div key={d.label} className="bar-row">
      <span className="bar-label">{d.label}</span>
      <div className="bar-track"><span className={`bar-fill p-${d.tone || 'blue'}`} style={{width: `${((Number(d.value) || 0) / max) * 100}%`}}/></div>
...
export function Donut({value, tone='green', label}) {
  ...
  return <div className="donut">
    <svg viewBox="0 0 140 140" role="img" aria-label={`${label || 'Progress'}: ${safeValue}%`}>
      ...
      <circle cx="70" cy="70" r={r} fill="none" stroke={stroke} strokeWidth="14" strokeLinecap="round"
        strokeDasharray={c} strokeDashoffset={c - (c * safeValue) / 100} transform="rotate(-90 70 70)"/>
      <text x="70" y="66" textAnchor="middle" className="donut-value">{safeValue}%</text>
