...
const emptyForm = {
  date: '2026-09-15', time: '09:00', type: 'Near Miss', severity: 'Medium', description: '',
...
export default function Incident() {
  ...
  const filtered = useMemo(() => incidents.filter(i => {
    ...
    const matchSearch = !q || [i.id, i.description, i.unit, i.location, i.reporter, i.investigator].join(' ').toLowerCase().includes(q)
    return matchSearch &&
      (filterType === 'All' || i.type === filterType) &&
      (filterStatus === 'All' || i.status === filterStatus) &&
      (filterUnit === 'All' || i.unit === filterUnit)
  }), [incidents, search, filterType, filterStatus, filterUnit])

  const active = incidents.filter(i => i.status !== 'Closed').length
  const investigation = incidents.filter(i => ['Investigation', 'RCA'].includes(i.status)).length
  const critical = incidents.filter(i => i.severity === 'Critical' && i.status !== 'Closed').length
  const overdue = incidents.filter(isOverdue).length
  const closed = incidents.filter(i => i.status === 'Closed').length
  const closureRate = incidents.length ? Math.round((closed / incidents.length) * 100) : 0

  const byType = types.map(typeName => ({
    label: typeName,
    value: incidents.filter(i => i.type === typeName).length,
  ...
  function saveIncident(e) {
    e.preventDefault()
    if (!form.description.trim() || !form.location.trim() || !form.reporter.trim() || !form.investigator.trim()) {
      setNotice('Lengkapi deskripsi, lokasi, pelapor dan investigator terlebih dahulu.')
      return
    }
    ...
    setForm(emptyForm)
    setModalOpen(false)
    setNotice(`${id} berhasil dilaporkan.`)
  ...
  function syncCorrectiveAction(item, targetStatus) {
    ...
    try {
      ...
      const id = `CA-${item.id}`
      const exists = actions.some(a => a.id === id || a.sourceId === item.id)
      if (!exists) {
        const action = {
          id,
          source: 'Incident',
        ...
        localStorage.setItem('sinshe-corrective-actions', JSON.stringify([action, ...actions]))
      }
    } catch {}
  }
  ...
  function exportCSV() {
    const header = ['ID', 'Tanggal', 'Jam', 'Tipe', 'Severity', 'Deskripsi', 'Unit', 'Lokasi', 'Pelapor', 'Investigator', 'Status', 'Immediate Action', 'Root Cause', 'Corrective Action', 'Due Date']
    const rows = filtered.map(i => [i.id, i.date, i.time, i.type, i.severity, i.description, i.unit, i.location, i.reporter, i.investigator, i.status, i.immediateAction, i.rootCause, i.correctiveAction, i.dueDate])
  ...
  return <Shell title="Incident Management" subtitle="Pelaporan insiden, investigasi, root cause analysis dan tindak lanjut korektif terintegrasi.">
    ...
    <div className="stats-grid four">
      <StatCard label="Insiden Aktif" value={active} hint={`${incidents.length} total register`} tone="blue" icon={<Siren/>}/>
    ...
    <div className={styles.headerRow}>
      <div><h2>Incident Register</h2><p>Kelola laporan awal sampai investigasi, RCA, corrective action dan closure.</p></div>
      <div className={styles.buttons}>
        <button className={styles.secondary} onClick={exportCSV}><Download size={17}/> Export CSV</button>
        <button className={styles.primary} onClick={() => setModalOpen(true)}><Plus size={18}/> Laporkan Insiden</button>
      </div>
    </div>

    <Panel>
      <div className={styles.filters}>
        <label className={styles.search}><Search size={17}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cari ID, deskripsi, lokasi, investigator..."/></label>
      ...
      <div className="table-wrap"><table>
        <thead><tr><th>ID</th><th>Tanggal</th><th>Tipe</th><th>Insiden</th><th>Unit / Lokasi</th><th>Severity</th><th>Investigator</th><th>Due Date</th><th>Status</th><th>Aksi</th></tr></thead>
        <tbody>
          {filtered.map(i => <tr key={i.id} className={isOverdue(i) ? styles.overdueRow : ''}>
            <td><b>{i.id}</b><small className={styles.block}>{i.time}</small></td>
            <td>{fmt(i.date)}</td>
            <td><Badge tone={tone(i.type)}>{i.type}</Badge></td>
            <td className={styles.descCell}><b>{i.description}</b><small>Pelapor: {i.reporter}</small></td>
            <td><b>{i.unit}</b><small className={styles.block}>{i.location}</small></td>
            <td><Badge tone={tone(i.severity)}>{i.severity}</Badge></td>
            <td>{i.investigator}</td>
            <td><span className={isOverdue(i) ? styles.overdueText : ''}>{fmt(i.dueDate)}</span></td>
          ...
          {!filtered.length && <tr><td colSpan="10" className={styles.empty}>Tidak ada insiden yang sesuai filter.</td></tr>}
        </tbody>
      </table></div>
    </Panel>
    ...
    <div className="dashboard-split mt">
      ...
      <Panel title="Prioritas Investigasi">
        <div className={styles.priorityBox}><AlertTriangle size={24}/><div><b>{critical} critical incident aktif</b><p>{overdue} tindak lanjut melewati due date. Pastikan evidence, RCA dan corrective action terdokumentasi sebelum closure.</p></div></div>
        <div className="ai-card" style={{marginTop:12}}><Activity/><div><b>Integrated Workflow</b><p>Incident yang masuk tahap Action Pending otomatis membuat record di Corrective Action Tracking jika corrective action tersedia.</p></div></div>
      </Panel>
    </div>
    ...
    {modalOpen && <div className={styles.backdrop} onMouseDown={e => { if (e.target === e.currentTarget) setModalOpen(false) }}>
      <form className={styles.modal} onSubmit={saveIncident}>
        <div className={styles.modalHeader}><div><span>SINSHE 2.0</span><h2>Laporkan Insiden</h2><p>Catat laporan awal dan penanggung jawab investigasi.</p></div><button type="button" onClick={() => setModalOpen(false)}><X size={20}/></button></div>
    ...
    {selected && <div className={styles.backdrop} onMouseDown={e => { if (e.target === e.currentTarget) setSelected(null) }}>
      <div className={styles.detailModal}>
        <div className={styles.modalHeader}><div><span>{selected.id}</span><h2>{selected.type}</h2><p>{selected.unit} • {selected.location}</p></div><button onClick={() => setSelected(null)}><X size={20}/></button></div>
        ...
        <div className={styles.detailGrid}>
          <div><span>Tanggal / Jam</span><b>{fmt(selected.date)} • {selected.time}</b></div>
          <div><span>Pelapor</span><b>{selected.reporter}</b></div>
          <div><span>Investigator</span><b>{selected.investigator}</b></div>
          <div><span>Due Date</span><b>{fmt(selected.dueDate)}</b></div>
        </div>
        <div className={styles.detailSection}><span>Kejadian</span><p>{selected.description}</p></div>
        <div className={styles.detailSection}><span>Immediate Action</span><p>{selected.immediateAction || '-'}</p></div>
        <div className={styles.detailSection}><span>Root Cause</span><p>{selected.rootCause || 'Belum dilengkapi'}</p></div>
        <div className={styles.detailSection}><span>Corrective Action</span><p>{selected.correctiveAction || 'Belum dilengkapi'}</p></div>
